...
void *thread_select(){ //T2
	int i, j, s_temp, atividade_socket, conexao_aceita=0, b_size;
	...
	while(1){
		...
		for(i=0; i<MAX_CLIENTES; i++){
			...
			if(s_temp > 0)
				...
				if(s_temp > max_socketfd)		//necessario pro select, pegar o maior numero do file descriptor
					max_socketfd = s_temp;
		}
			...
			if(FD_ISSET(s_temp, &readfd)){
				...
				if(b_size>0){ //msg recebida
					...
					if(strcmp(temp,"SEND")==0){
						...
						fprintf(stderr, "MSG_ENVIADA: [%s]", buffer);
